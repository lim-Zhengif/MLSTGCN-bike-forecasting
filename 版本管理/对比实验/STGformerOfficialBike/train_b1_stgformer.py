...
from models.multi_relation_graph_propagate import (
    B1STGformer as STGformerOfficialBike,
    build_b1_from_checkpoint as build_model_from_checkpoint,
    B1_MODEL_ID,
)
...
def main():
    parser = argparse.ArgumentParser(description="Train B1 static multi-relation STGformer from scratch")
    ...
    parser.add_argument("--external_order", type=int, default=2)
    parser.add_argument("--initial_strength", type=float, default=0.01)
    ...
    parser.add_argument("--wandb_tags", default="stgformer,official-core,b1,top150,hist168,pred3")
    args = parser.parse_args()
    if args.epochs < 1:
        raise ValueError("epochs must be positive")
    if args.output_dir is None:
        args.output_dir = str(
            DEFAULT_RESULT_ROOT / ("b1_static_multirelation_seed%d_bs%d" % (args.seed, args.batch_size))
        )
    ...
    supports, relation_names, graph_audit = load_relations(graph_dir, num_nodes)
    ...
    model = STGformerOfficialBike(
        supports=supports, relation_names=relation_names, relation_audit=graph_audit,
        external_order=args.external_order, initial_strength=args.initial_strength,
        num_nodes=num_nodes,
        in_steps=hist_len,
        out_steps=pred_len,
        input_dim=in_dim,
    ...
    best_epoch = -1
    bad_epochs = 0
    checkpoint_path = output_dir / "best_stgformer_b1.pt"
    history = []
    ...
    summary["model_config"] = model.export_config()
    summary["parameter_count"] = sum(p.numel() for p in model.parameters())
    summary["initialization"] = "same-seed from scratch; no B0/G0 checkpoint warm start"
    pd.DataFrame(model.diagnostics()).to_csv(output_dir / "gate_diagnostics.csv", index=False)
