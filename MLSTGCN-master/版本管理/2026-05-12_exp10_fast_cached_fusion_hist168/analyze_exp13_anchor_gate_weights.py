...
def main():
    ...
    parser.add_argument("--project", default="bike_hourly_safe_inventory_top150_exp13_anchor_hour_context_gated_multigraph_hist168_pred6_bs8_seed0")
    ...
    parser.add_argument("--tag", default="exp13_anchor_gate", help="Prefix used for output filenames.")
    ...
    output_dir = (
        ...
        if args.output_dir
        else PROJECT_ROOT / ANALYSIS_DIR / "2026-05-16_exp13_anchor_gate_weight_analysis"
    )
    ...
    gate_path = output_dir / ("%s_by_sample.csv" % args.tag)
    ...
    by_anchor_path = output_dir / ("%s_by_anchor.csv" % args.tag)
    ...
    summary = {
        ...
        "order_dir": str(Path(args.order_dir) if args.order_dir else PROJECT_ROOT / FEB_DATA_DIR / ORDER_SUBDIR),
        ...
        "context_gate_anchor_hour": bool(graph_config.get("context_gate_anchor_hour", False)),
        "context_gate_anchor_hour_index": int(graph_config.get("context_gate_anchor_hour_index", -1)),
        "context_gate_anchor_embed_dim": int(graph_config.get("context_gate_anchor_embed_dim", 8)),
        "context_gate_anchor_od_prior": float(graph_config.get("context_gate_anchor_od_prior", 0.0)),
        "context_gate_scope": str(graph_config.get("context_gate_scope", "all")),
    ...
    summary_path = output_dir / ("%s_summary.json" % args.tag)
